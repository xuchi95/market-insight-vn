...
export const Route = createRootRouteWithContext<{ queryClient: QueryClient }>()({
  ...
  head: ({ loaderData }) => ({
    ...
    scripts: [
      // Google AdSense loader — verify publisher ca-pub-1602622310716271.
      // Mã `<ins class="adsbygoogle">` đặt trong <AdSlot> hoặc dán qua admin
      // code-injection sẽ tự động được fill bởi script này.
      {
        async: true,
        src: `https://pagead2.googlesyndication.com/pagead/js/adsbygoogle.js?client=${import.meta.env.VITE_ADSENSE_CLIENT || "ca-pub-1602622310716271"}`,
...
function RootShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <html lang="vi" className="dark">
      <head>
        {/* Blocking theme bootstrap — chạy trước paint để tránh flash dark→light khi F5.
            Đọc localStorage('mw-theme') và set class trên <html> trước khi React hydrate. */}
        <script
          dangerouslySetInnerHTML={{
            __html:
              "(function(){try{var t=localStorage.getItem('mw-theme');var r=document.documentElement;var resolved;if(t==='light'||t==='dark'){resolved=t;}else{resolved=(window.matchMedia&&window.matchMedia('(prefers-color-scheme: dark)').matches)?'dark':'light';}r.classList.toggle('dark',resolved==='dark');r.classList.toggle('light',resolved==='light');}catch(e){}})();",
          }}
        />
        <style>{`html.dark{--background:#0d0d0d;--foreground:#f5f0df;--card:#1a1a1a;--card-foreground:#f5f0df;--popover:#1a1a1a;--popover-foreground:#f5f0df;--primary:#c9a84c;--primary-foreground:#0d0d0d;--secondary:#221f1a;--secondary-foreground:#f5f0df;--muted:#1f1d18;--muted-foreground:#c8b98f;--accent:#2d2820;--accent-foreground:#f5f0df;--border:#4c463a;--input:#221f1a;--ring:#c9a84c;--gold:#c9a84c;--gold-light:#f0d78c;--gold-foreground:#0d0d0d;--up:#4ab377;--down:#e85d3a;--destructive:#e85d3a;--destructive-foreground:#fbf7ee;color-scheme:dark}html.light{--background:#f3eee4;--foreground:#2a241b;--card:#fbf7ee;--card-foreground:#2a241b;--popover:#fbf7ee;--popover-foreground:#2a241b;--primary:#8a6a1f;--primary-foreground:#fbf7ee;--secondary:#ebe5d6;--secondary-foreground:#2a241b;--muted:#ebe5d6;--muted-foreground:#5b5240;--accent:#e2dac8;--accent-foreground:#2a241b;--border:#d6cdb8;--input:#e2dac8;--ring:#8a6a1f;--gold:#8a6a1f;--gold-light:#b5904a;--gold-foreground:#fbf7ee;--up:#4d8e5d;--down:#c0432a;--destructive:#c0432a;--destructive-foreground:#fbf7ee;color-scheme:light}html{background:var(--background)}body{margin:0;background:var(--background);color:var(--foreground);font-family:"Be Vietnam Pro",ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}*{box-sizing:border-box}`}</style>
        <HeadContent />
        {headInjections}
      </head>
      <body>
