...
async function generateAndSendRecovery(email: string, redirectTo: string) {
...
export const requestPasswordRecovery = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    try {
      await generateAndSendRecovery(data.email, data.redirectTo);
    } catch (err) {
      // Silently log — do not leak whether the email exists.
      console.error("[password-recovery] failed:", err);
    }
    return { ok: true };
  });

/** Internal helper for admin-triggered recovery (caller must already be authorized). */
export async function sendRecoveryEmailFor(email: string, redirectTo: string) {
  await generateAndSendRecovery(email, redirectTo);
}